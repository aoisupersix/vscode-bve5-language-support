...
export class MapDoc {
    ...
    constructor(
        ...
        this.syntaxes.push(new MapSyntax(syntaxType, mapElement1Name, mapElement2Name, funcName, document, params))
    }
    ...
    public isMatchMapElement1(mapElement1Text: string): boolean {
        ...
        if (type === MapSyntaxType.Syntax1) {
            // シンタックス1はキーがない
            return (
                mapElement1Text.match(new RegExp(String.raw`^${this.syntaxes[0].getMapElement1Name()}$`, 'i')) !== null
            )
        }
        return (
            mapElement1Text.match(new RegExp(String.raw`^${this.syntaxes[0].getMapElement1Name()}\[.*\]$`, 'i')) !==
            null
        )
    }
    ...
    public getMapElement1CompletionItem(): vscode.CompletionItem {
        const item = new vscode.CompletionItem(this.syntaxes[0].getMapElement1Name(), vscode.CompletionItemKind.Class)
    ...
    public getMapElement2CompletionItem(): vscode.CompletionItem {
        const item = new vscode.CompletionItem(this.syntaxes[0].getMapElement2Name(), vscode.CompletionItemKind.Class)
    ...
    public getFunctionCompletionItem(): vscode.CompletionItem {
        const item = new vscode.CompletionItem(this.syntaxes[0].getFuncName(), vscode.CompletionItemKind.Function)
    ...
    public getFunctionHover(range: vscode.Range): vscode.Hover {
        return new vscode.Hover(
            [new vscode.MarkdownString(`${this.syntaxes[0].getSyntaxDisplayName()}`), this.syntaxes[0].getDocument()],
            range
        )
    }
}
